      ...
      loadRoomTypeCalendarData: function loadRoomTypeCalendarData(startDate, monthsCount, roomTypeId, isShowPrices, isTruncatePrices, isShowPricesCurrency) {
        ...
        if (startLoadingDate.getTime() < endLoadingDate.getTime()) {
          ...
          if (!this._activeAjaxRequests[ajaxRequestKey]) {
            ...
            console.log('START LOADING: ' + JSON.stringify([roomTypeId, isShowPrices, isTruncatePrices, isShowPricesCurrency]) + ': ' + formattedStartLoadingDate + ' - ' + formattedEndLoadingDate);
            var ajaxRequestPromise = $.ajax({
              ...
              success: function success(response) {
                ...
                console.log('DATA LOADED: ' + JSON.stringify([roomTypeId, isShowPrices, isTruncatePrices, isShowPricesCurrency]) + ': ' + formattedStartLoadingDate + ' - ' + formattedEndLoadingDate + ' TIME: ' + (executionTime / 1000).toFixed(2) + ' sec', response.data);
      ...
      init: function init($calendarElement, args) {
        ...
        this.$calendarElement.hide().datepick({
          ...
          onChangeMonthYear: function onChangeMonthYear(year, month) {
            ...
            MPHB.ajaxApiHelper.loadRoomTypeCalendarData(new Date(year, month - 1, 1), MPHB._data.settings.numberOfMonthDatepicker, self.roomTypeId, self.isShowPrices, self.isTruncatePrices, self.isShowPricesCurrency, function () {
              self.$calendarElement.addClass('mphb-loading');
            }, function () {
              self.doAfterNewCalendarDataLoaded(self);
              self.$calendarElement.removeClass('mphb-loading');
