...
class RoomAvailabilityHelper {
	...
	public static function getRoomTypeAvailabilityStatus( int $roomTypeOriginalId, \DateTime $date ) {
		...
		if ( 0 < $roomTypeOriginalId ) {
			...
			$allRoomTypeIds = MPHB()->getRoomTypeRepository()->findRoomTypeIds();
			...
			foreach ( $allRoomTypeIds as $roomTypeId ) {
				...
				if ( in_array( $formattedDateYmd, $datesRates ) &&
					0 < static::getAvailableRoomsCountForRoomType( $roomTypeId, $date )
				) {
	 ...
	 * @return \DateTime
	 */
	public static function getFirstAvailableCheckInDate( int $roomTypeOriginalId = 0 ) {

		$firstAvailableDate = new \DateTime('yesterday');
		$maxCheckDatesCount = 365;
